...
class HrAttendance(models.Model):
    ...
    def _is_early_checkout(self):
        for rec in self:
            ...
            worked_hrs = sum(rec.employee_id.attendance_ids.filtered(lambda x: x.check_in.date() == check_in_day).mapped('worked_hours'))
                ...
                lambda
                    l: l.state == 'validate' and l.request_unit_half and l.request_date_from <= rec.check_in.date() <= l.request_date_to
                       and ((l.request_date_from == rec.check_in.date() and l.request_date_from_period == "am")
                            or (l.request_date_to == rec.check_in.date() and l.request_date_to_period == "am")
                            )
            )
            period = 'afternoon' if check_morning_half_day else 'morning'
            actual_check_in_calendar_resource_attendance = calendar_resource_attendances.filtered(lambda x: x.day_period == period)
    ...
    def _deserve_penalty(self):
        for attendance in self:
            ...
            if not allowed_count or allowed_count == 0:
                return False
            if not leave_type_id:
                raise ValidationError(_("Oops! It looks like the Penalty Leave Type hasn't been set up yet. Please contact your administrator to get it configured."))
            penalty_leaves = employee.leave_ids.filtered(lambda x: x.state in ['validate'] and x.is_penalty_leave).sorted(lambda x: x.request_date_from, reverse=True)
            ...
            late_attendances = employee.attendance_ids.filtered(
                lambda x: x.late_reason != False and not x.leave_id and x.check_in and fields.Datetime.context_timestamp(x,x.check_in).astimezone(emp_tz) > last_leave_date)
        ...
        for attendance in self:
            if attendance._deserve_penalty():
                try:
                    allowed_count, leave_type_id = attendance._get_penalty_params()
                    if not leave_type_id:
                        raise (_("Penalty leave type is not configured in attendance settings,Please contact administrator."))
