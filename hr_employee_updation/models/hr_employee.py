...
class HrEmployee(models.Model):
    ...
    joining_date = fields.Date(compute='_compute_joining_date',
                               ...
                               help="Employee joining date computed from the"
                                    " contract start date")
    contract_end_date = fields.Date(compute='_compute_joining_date',
                               string='Contract End Date', store=True,
                               help="Employee end date computed from the"
                                    " contract end date")
    ...
    pan_card_no = fields.Char("Income Tax Number (PAN)")
    uan_no = fields.Char("Universal Account Number (UAN)")
    ...
    def _compute_joining_date(self):
        ...
        for employee in self:
            ...
            employee.contract_end_date = min(
                employee.contract_id.mapped('date_end')) \
                if employee.contract_id else False
