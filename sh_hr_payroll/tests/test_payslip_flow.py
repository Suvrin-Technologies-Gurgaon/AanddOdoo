...
class TestPayslipFlow(TestPayslipBase):
    def test_00_payslip_flow(self):
        ...
        data, data_format = self.env.ref("sh_hr_payroll.action_report_payslip").render(
            richard_payslip.ids
        )

        # I print the payslip details report
        data, data_format = self.env.ref("sh_hr_payroll.payslip_details_report").render(
            richard_payslip.ids
        )
        ...
        context = {
            ...
            "active_ids": [self.ref("sh_hr_payroll.hr_houserent_register")],
        }
