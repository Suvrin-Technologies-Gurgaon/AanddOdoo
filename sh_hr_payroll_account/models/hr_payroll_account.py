...
class HrPayslip(models.Model):
    ...
    journal_id = fields.Many2one('account.journal', 'Salary Journal', required=True,default=lambda self: self.env['account.journal'].search([('type', '=', 'general')], limit=1))
    ...
    def action_payslip_done(self):
        ...
        for slip in self:
            ...
            name = _("Payslip of %(slip)s", slip=slip.employee_id.name)
            move_dict = {
                'narration': name,
                'ref': slip.number,
            ...
            if currency.compare_amounts(credit_sum, debit_sum) == -1:
                ...
                if not acc_id:
                    name = _("Payslip of %(slip)s", slip=slip.employee_id.name)
                    raise UserError(_("The Expense Journal '%(slip)s' has not properly configured the Credit Account!",
                ...
                if not acc_id:
                    raise UserError(_("The Expense Journal '%(slip)s' has not properly configured the Debit Account!",
                                      slip.journal_id.name))
                adjust_debit = (0, 0, {
                    'name': _('Adjustment Entry'),
                    'partner_id': False,
